...
* @param delta tick time.
*/
void Physics::Gravity(Player* p, int** collidableArray, float delta)
{
	//Gravity Start-----------------------------------------------------------------------------------------------------
...
bool Physics::Grounded(Player* p, int** collidableArray)
{
	...
	int temp = collidableArray[playerSouthCoord][playerArrayCoordX];

	if (collidableArray[playerSouthCoord][playerArrayCoordX] > 0)
		return p->GetApexCheck();
	return false;
}
...
bool Physics::Roofed(Player* p, int** collidableArray)
{
	...
	{
		int tileId = collidableArray[playerNorthCoord][playerArrayCoordX];
		return true;
	}
	return false;
}
