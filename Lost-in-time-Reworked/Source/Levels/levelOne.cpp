...
LevelOne::LevelOne(sf::RenderWindow& window)
{
	...
	************************************************/
	...
	interactionArray = new int*[ArraySize];
	interactionArray[0] = new int[ArraySize * ArraySize];
	for (int i = 1; i < ArraySize; i++)
	{
		collidableArray[i] = collidableArray[i - 1] + ArraySize;
		interactionArray[i] = interactionArray[i - 1] + ArraySize;
	}

	/*************************************************
	* Load map information from JSON into object list
	************************************************/
	if (!Map::Load("data/testMap.json", objects, collidableArray, spawnpointList, interactionArray, teleports))
	...
	*************************************************/
	...
	//player = new Player(200, 200, &window);
...
bool LevelOne::Tick(Machine& machine)
{
	...
	***********************/
	...
	time = timer->getElapsedTime().asSeconds() + penaltyTime - EscMenuTime;
	std::string tempForTime = std::to_string(time);
	...
	*******************/
	...
	// uncomment to see the amount of time rendering takes
...
*          false if to quit
************************************************/
/*bool LevelOne::EscMenu(Machine& machine)
{
	music->music.pause();
...
**********************************************/
/*bool LevelOne::SaveGame(int selectedSave)
{
...
***********************************************************/
/*int LevelOne::menuSelected(std::string menu)
{
...
**********************************/
void LevelOne::LoadImages()
{
	// dummy variable to help with menu
	int nothing;
...
*********************************************/
sf::Texture* LevelOne::LoadTexture(std::string path, int &menuAmount)
{
	//temp texture
